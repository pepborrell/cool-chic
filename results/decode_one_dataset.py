...
parser.add_argument(
    "configuration", help=f"Possible values {POSSIBLE_DATASETS_PER_CONFIG.keys()}."
)
parser.add_argument(
    "dataset_name", help=f"Possible values {POSSIBLE_DATASETS_PER_CONFIG}."
)
args = parser.parse_args()


assert args.configuration in POSSIBLE_DATASETS_PER_CONFIG.keys(), (
...
bitstream_path = os.path.join(
    current_dir_path, args.configuration, args.dataset_name, "bitstreams/"
)
...
for idx, cur_bitstream in enumerate(all_bitstreams):
    ...
    cur_decoded_path = (
        f'{decoded_image_path}{cur_bitstream.split("/")[-1].split(".")[0]}.{extension}'
    )

...
from coolchic.enc.training.presets import Preset
...
class FrameEncoderManager:
    ...
    n_itr: int = int(
        1e5
    )  # Maximum number of training iterations for a **single** phase
    ...
    preset: Preset = field(
        init=False
    )  # It contains the learning rate in the different phase

    # ----- Monitoring
    idx_best_loop: int = field(
        default=0, init=False
    )  # Index of the loop which gives the best results (i.e. the best_loss)
    best_loss: float = field(
        default=1e6, init=False
    )  # Overall best loss (for all loops)
